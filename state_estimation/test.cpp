...
int main()
{
    ...
    Matrix6d Q = 1e-4 * Matrix6d::Identity();  // to be tuned
    ...
    // Introduce modeling errors in inertial parameters
    M *= 0.9;

    // White noise generators
    double random_force_noise = 1000;  
    ...
    // std::normal_distribution<double> force_dist(10, random_force_noise);
    ...
    Vector3d gravity_bias = Vector3d(0, 0, -9.81);  // gravity bias in the accel readintg
    ...
    if (test == 0) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            accel_sensor += gravity_bias;  // add gravity bias reading 

            // Update sensor information and filter (frequency from n_skip)
            if (i % n_skip == 0) {
                // Naive integration
                integrated_velocity += (accel_sensor - gravity_bias) * filter_dt;  // assumes perfect inertial accel measurement without gravity bias
    ...
    // plot0.drawCurve(filter_time_vector, vx_naive).label("vx naive integrate").lineWidth(2);
    ...
    // plot1.drawCurve(filter_time_vector, vy_naive).label("vy naive integrate").lineWidth(2);
    // // plot0.drawCurve(time_vector, vx_naive).label("vx integrated").lineWidth(2);
    ...
    // plot2.drawCurve(filter_time_vector, vz_naive).label("vz naive integrate").lineWidth(2);
    // // plot0.drawCurve(time_vector, vx_naive).label("vx integrated").lineWidth(2);
